...
/**
 * check_ep - Executes the corresponding function based on the opcode.
 * @top_up: Double pointer to the head of the stack.
 * @arr_ep: Array of strings representing the opcode and its arguments.
 * @error_ep: Boolean indicating if there is an error in opcode usage.
 * @mode_ep: Pointer to a boolean indicating the stack or queue mode.
 * @line_ep: Line number being executed from the script file.
 */
void check_ep(stack_t **top_ep, char **mer_pi, bool error_ep, bool *mode_ep, unsigned int line_ep)
{
